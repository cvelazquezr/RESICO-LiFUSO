...
/** Object to load in memory some of the predefined models. Convenient for
  * optimization on execution.
  */
object PredefinedModels:

  /** Loads the defined models to perform a lemmatization on the text.
    * @return
    *   A tuple containing the posTagger and lemmatizer models.
    */
  def lemmatization(): (POSTaggerME, DictionaryLemmatizer) =
    ...
    // Loading Lemmatizer
    val streamLemmatizer: InputStream = new FileInputStream(
      "lib/en-lemmatizer.txt"
    )
    val dictionaryLemmatizer: DictionaryLemmatizer = new DictionaryLemmatizer(
      streamLemmatizer
    )
