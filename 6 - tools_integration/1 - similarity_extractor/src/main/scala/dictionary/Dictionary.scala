...
class Dictionary(library: Library, dataFolder: String):
  ...
  def createDictionary(): Unit =
    ...
    library.versions
      ...
      .foreach(version =>
        println(
          s"Analysing triplet ${library.groupID} -> ${library.artifactID} -> $version"
        )

        // 1. Download the JAR file with a triple combination
        val jarFile: JARFile =
          new JARFile(library.groupID, library.artifactID, version)
    ...
    val pw: PrintWriter = new PrintWriter(
      new File(
        s"$dataFolder/dictionaries/${library.groupID}" +
  ...
  def increaseDictionary(
      filePath: String,
      dictionary: MMap[String, Array[String]]
    ...
    if file.exists() then
      ...
      entries
        ...
        .foreach(entry =>
          ...
          if !entryName.equals("module-info.class") then
            ...
            if javaClass.isPublic then
              val className: String =
                entryName.split("/").last.split("\\.").head

              val methods: Array[Method] = javaClass.getMethods
              val publicMethodNames: Array[String] =
                methods.filter(_.isPublic()).map(_.getName()).distinct

              if dictionary.contains(className) then
                val currentMethods: Array[String] = dictionary(className)
                dictionary(className) =
                  currentMethods.concat(publicMethodNames).distinct
              else dictionary(className) = publicMethodNames
        )
      zipFile.close()
    dictionary
