...
object VectorAnalysis:
  ...
  /** Creates TFIDF vectorizers for the usages passed as arguments. Current
    * implementation is designed to return Word2Vec representation models of the
    * data.
    *
    * @param usages
    *   The collection of API usages consisting in different BoWs. Each of them
    *   with the characteristics of a Stack Overflow post.
    * @return
    *   A Map where the keys are the names of the models and the values
    *   correspond to the actual models.
    */
  def createTFIDFModel(usages: List[API_Usage_Text]): List[TfidfVectorizer] =
    ...
    val allTitles: List[List[String]] =
      usages.map(_.bows).filter(_.nonEmpty)
...
//    val allQuestionBodiesModel: TfidfVectorizer = VectorAnalysis.tfidfModel(allQuestionBodies)
    ...
    val allAnswerBodies: List[List[String]] =
      usages.map(_.bows).filter(_.nonEmpty)
    val allAnswerBodiesModel: TfidfVectorizer =
      VectorAnalysis.tfidfModel(allAnswerBodies)

    List(allTitlesModel, allAnswerBodiesModel)

  /** This method transforms a list of Bag of Words (passed as argument) to a
    * list of vectors. Several techniques for vector transformation from text
    * could be adopted. Word2Vec is adopted in this case for all words in the
    * corpus.
    *
    * @param allDocuments
    *   The list of all documents to be transformed.
    * @return
    *   The vector representation of the documents.
    */
  def word2vecModel(allDocuments: List[List[String]]): Word2Vec =
    val labelledDocuments = new util.ArrayList[LabelledDocument]()

    var index: Int = 0
    for document <- allDocuments do
  ...
  /** Given of mapping between words and their vector representation, transforms
    * a document to its vector representation.
    *
    * @param document
    *   A list of words to be transformed.
    * @param model
    *   A TF-IDF model able to transform the text passed as an argument into a
    *   single vector.
    * @return
    *   A single vector representing the document passed as an argument.
    */
  def document2Vector(
    ...
    if collectionWords.isEmpty || (collectionWords
        .size() == 1 && collectionWords.get(0).trim.isEmpty)
    then Array()
    else
      val wordsTransformed: Array[Double] =
        model.transform(collectionWords).mean(0).toDoubleVector

      wordsTransformed

  /** This method transforms a list of Bag of Words (passed as argument) to a
    * list of vectors. Several techniques for vector transformation from text
    * could be adopted. TF-IDF is adopted in this case for all words in the
    * corpus.
    *
    * @param allDocuments
    *   The list of all documents to be transformed.
  ...
  def tfidfModel(allDocuments: List[List[String]]): TfidfVectorizer =
    val collectionSentences: util.ArrayList[String] =
      new util.ArrayList[String]()
    allDocuments.foreach(document =>
      collectionSentences.add(document.mkString(" "))
    )
    val sentenceIterator: SentenceIterator = new CollectionSentenceIterator(
      collectionSentences
    )
  ...
  /** This method transforms a list of textual Bag of Words (passed as argument)
  ...
  /** Averages the vectors in an API usage
    *
    * @param usages
    *   The API vector usages
    * @return
    *   A list of API usages with their vectors combined
    */
  def averageCombination(
      usages: Array[API_Usage_Vector]
  ): Array[API_Usage_Vector_Combined] =
    usages.map(usage =>
      // In the case of TF-IDF vectors could have different sizes
      var vectors: Array[Array[Double]] = usage.vectors
      if vectors.length == 1 && vectors.head.isEmpty then
        vectors = Array(Array.fill(100)(0.0))

      val allArrays: Array[Array[Double]] = vectors

      val allArraysStandard: Array[Array[Double]] =
        standardizeVectors(allArrays)
  ...
  /** Transform all vectors to be of the same size
    *
    * @param matrix
    *   A matrix containing all vectors to be transformed
    * @return
    *   A standardize version of all vectors
    */
  ...
  /** Calculates the similarities between the vectors in specific combinations.
    *
    * @param combinations
    *   The array with the combinations made
    * @return
    *   A matrix with the values of similarities between the vectors in the
    *   combinations.
    */
  def similarityCombinations(
      combinations: Array[Array[Double]]
  ): Array[Array[Double]] =
  ...
  def jaccardSimilarity(callsList: List[API_Usage]): Array[Array[Double]] =
    callsList
      .map(referenceAPIs =>
        callsList
          .map(accessAPIs =>
            val setReferences: Set[String] = referenceAPIs.bows.toSet
            val setAccess: Set[String]     = accessAPIs.bows.toSet

            setReferences
              .intersect(setAccess)
              .size
              .toDouble / setReferences.union(setAccess).size.toDouble
          )
          .toArray
      )
      .toArray
